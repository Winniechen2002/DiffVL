...
layout(set = 0, binding = 14) uniform usampler2D samplerPointSeg;
layout(set = 0, binding = 15) uniform sampler2D samplerPointColor;

layout(set = 0, binding = 16) uniform sampler2D samplerPointDepthLinear;
...
layout(location = 6) out vec4 outPointK;
...
void main() {
  ...
  int use_point_color = 0;
  float d = min(d0, d2);
  outColor = outColor0 * factor + outColor2 * (1 - factor);
  outPosition = outPos0;

  // depth composite for 02 and p
  outPointK = vec4(-texture(samplerPointDepthLinear, inUV).x,0,0,1);
  if (pointDepth < 0) {
    factor = step(d, dp);
    d = min(d, dp);
    //float factor2 = (1-pointColor.w) * (1-factor);
    float factor2 = 1 - factor;
    if(factor2 > 0){
      factor2 = factor2 * max(min(1.-texture(samplerPointColor, inUV).w, 1.), 0.);
    }
    outColor = outColor * (1-factor2) + pointColor * factor2;
    if(factor < 0.5) use_point_color = 1;
  ...
  if(use_point_color > 0) {
    outSegmentation = texture(samplerPointSeg, inUV);
  }
