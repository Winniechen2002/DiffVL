...
namespace maniskill {
        ...
        vec3 const &gx)
    {
        ...
        if(type == 0){
            ...
            float x=shape_args.w, y=shape_args.x;
            float px = ::abs(gx.x);
            float py = gx.y;
            float pz = gx.z;

            // x is distance of the centroid and the triangle's vertex.
            float d1 = px*0.866025+py*0.5 - x*0.5;
            //sdf = fmaxf(q.z-y,fmaxf(,-gx.y)-x*0.5);
            //assert(0);
    ...
    inline CUDA_CALLABLE vec3 shape_grad(
        ...
        if(type == 0){
            ...
            vec3 q = abs(gx);
            float x=shape_args.w, y=shape_args.x;
            //sdf = fmaxf(q.z-y,fmaxf(q.x*0.866025+y*0.5,-y)-x*0.5);
