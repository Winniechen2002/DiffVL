...
__global__ void compute_dist(
    ...
    quat *__restrict__ body_type_friction_softness_K,
    ...
    int dim)
{
  ...
  if (p < dim)
  {
    for (int body_id = 0; body_id < n_bodies; ++body_id)
    {
      ...
      quat type_friction_softness_K = body_type_friction_softness_K[body_id];
      quat shape_args = body_args[body_id];
      vec3 gx_b = spatial_transform_inv(bx, bq, particle_x[p]); // grid x in body frame
      if (compute_grad == 0)
      {
        dist[p * n_bodies + body_id] = shape_sdf(type_friction_softness_K, shape_args, gx_b);
      }
      else
      {
        //printf("gradient is not checked!\n");
        float grad_dist = dist_grad[p * n_bodies + body_id];
        vec3 unnormed_normal = shape_grad(type_friction_softness_K, shape_args, gx_b);
    ...
    int dim)
{
  ...
  if (tid < dim && grid_m[tid] > 1e-12)
  {
    ...
    for (int body_id = 0; body_id < n_bodies; ++body_id)
    {
      ...
      float friction = type_friction_softness_K.x;
      float softness = type_friction_softness_K.y;
      // if(tid ==0) printf("fric %f soft %f round %f\n", friction, softness, type_friction_softness_K.z);

      vec3 gx_b = spatial_transform_inv(bx, bq, gx); // grid x in body frame

      float dist = shape_sdf(type_friction_softness_K, shape_args, gx_b);
      ...
      if ((softness > 0 && influence > 0.1) || dist <= 0)
      {
        ...
        // printf("forward collision %d %d %d %d %d %f %f %f %f\n", body_id, n_bodies, grid_x, grid_y, grid_z, v_out.x, v_out.y, v_out.z);
        vec3 normal = normalized(shape_grad(type_friction_softness_K, shape_args, gx_b));
        ...
        vec3 grid_v_t = rel_v - min(normal_component, 0.f) * normal +  max(-dist, 0.f) * normal / dt * type_friction_softness_K.z;
  ...
  if (tid < dim && grid_m[tid] > 1e-12)
  {
    ...
    for (int body_id = n_bodies - 1; body_id >= 0; --body_id)
    {
      ...
      if ((softness > 0 && influence > 0.1) || dist <= 0)
      {
        ...
        float normal_component = dot(rel_v, normal);
        vec3 grid_v_t_in = rel_v - min(normal_component, 0.f) * normal +  max(-dist, 0.f) * normal / dt * type_friction_softness_K.z;
        ...
        float grad_dist2 = 0.; // - min(normal_component, 0.f) * normal +  max(-dist, 0.f) * normal / dt * type_friction_softness_K.z
        if (dist < 0.)
        {
          float weight = type_friction_softness_K.z / dt;
          grad_dist2 += -dot(normal, grad_grid_v_t) * weight;
          grad_normal += -dist * grad_grid_v_t * weight;
        }
        ...
        */
        ...
        auto found_shape_grad = shape_grad_backward(
            type_friction_softness_K, shape_args, gx_b,
        ...
        if (expdist <= 1)
        {
          float grad_dist = -softness * expdist * grad_influence + grad_dist2;
  ...
  if (n_bodies > 0)
  {
    ...
    while (j < 200 && dist < dist_limit && sdf_val > 1e-8)
    {
      ...
      for (int i = 0; i < n_bodies; ++i)
      {
        ...
        float tmp = shape_sdf(body_type_friction_softness_K[i], body_args[i], gx_b);
        if (tmp < sdf_val)
        {
          sdf_val = tmp;
          sdf_id = i;
    ...
    if (dist < closest && dist < dist_limit)
    {
      ...
      normal = body_rot[sdf_id] * normalized(shape_grad(body_type_friction_softness_K[sdf_id], body_args[sdf_id], gx_b));
  ...
  if (p < dim)
  {
    ...
    for (int sample_id = 0; sample_id < spp; ++sample_id)
    {
      ...
      while (depth < max_ray_depth)
      {
        ...
        float closest = next_hit(sdf_volume, box_min, box_max, color_volume, sdf_threshold, grid_dim, body_pos, body_rot, body_type_friction_softness_K, body_args, n_bodies, visualize_shape, ground_height, pos, d, roughness, color, normal, material);
        ...
        if (dot(normal, normal) > 1e-10)
        {
          ...
          if (use_directional_light)
          {
            ...
            if (dot_val > 0)
            {
              ...
              float dist = next_hit(sdf_volume, box_min, box_max, color_volume, sdf_threshold, grid_dim, body_pos, body_rot, body_type_friction_softness_K, body_args, n_bodies, visualize_shape, ground_height, pos, direct, roughness, color, normal, material);
      ...
      depth_buffer[p] += dot(first_depth * normalized(dir), vec3(0, 0, 1)) / spp;
    }
  ...
  */
      ...
      ivec3 const &grid_dim, int n_bodies, cudaStream_t stream)
  {
    ...
    launch_kernel(grid_op_v2, dim, stream,
                  (grid_m, grid_v_in, grid_body_v_in, grid_lower, gravity, body_pos, body_rot, next_body_pos, next_body_rot, body_type_friction_softness_K, body_args, dx, inv_dx, dt, ground_friction, out_grid_v, grid_dim, n_bodies, dim));
  }
      ...
      ivec3 const &grid_dim, int n_bodies, cudaStream_t stream)
  {
    ...
    launch_kernel(grid_op_v2_grad, dim, stream,
                  (grid_m, grid_v_in, grid_body_v_in, grid_lower, gravity, body_pos, body_rot, next_body_pos, next_body_rot, body_type_friction_softness_K, body_args,
  ...
  )
  {
    ...
    launch_kernel(render, dim, stream,
                  (sdf_volume, box_min, box_max, color_volume, sdf_threshold, body_pos, body_rot, body_type_friction_softness_K, body_args, camera_rot, camera_pos, camera_intrinsic, color_buffer, depth_buffer,
      ...
      cudaStream_t stream)
  {
    ...
    printf("%p %p %p\n", body_type_friction_softness_K, body_args, dist);
    printf("%p %p %p\n", particle_x_grad,body_pos_grad, body_rot_grad);
    printf("%p %p\n", dist_grad, stream);
    */
    launch_kernel(compute_dist, dim, stream,
                  (particle_x, body_pos, body_rot, body_type_friction_softness_K, body_args, dist, n_bodies,
