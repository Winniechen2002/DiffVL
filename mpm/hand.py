...
from mpm.hand_utils import JOINTS, JOINT_LIMITS, DEFAULT_INITIAL_QPOS, ACTUATOR_JOINT_MAPPING, \
...
class PD:
    def __init__(self, dt, Kp, Kd, output_limits):
        self.dt = dt
        self.Kp = Kp
        self.Kd = Kd
    ...
    def next(self, goal, curr):
        error = goal - curr
        d_input = curr - (self._last_input if (self._last_input is not None) else curr)

        new_output = self.Kp * error - self.Kd * (d_input / self.dt)
        new_output = new_output.clip(*self.output_limits)

        self._last_input = curr
...
class HandSimulator(MPMSimulator):
    def __init__(self,
        ...
        if action_scale is None:
            action_scale = [0.33 * 0.002] * self.n_actuators_per_hand + [0.01] * 3 + [0.015] * 3
            # action_scale[:2] = [0., 0.]
        ...
        ## PD Control ##
        ################
        self.pd_base_pos = None
        self.pd_joint_pos = None
    ...
    def sample_pts_inside_primitives(self, n_pts, mode=None, hot_start=None):
        ...
        def sample(num_points, hand_inds, center=None, hot_start=None):
        ...
        if mode == "dual":
            ...
            p1 = sample(n_pts_per_hand, LH_PRIM_RANGE, lh_center, hot_start)
            p2 = sample(n_pts_per_hand, RH_PRIM_RANGE, rh_center, hot_start)
            ...
            p = sample(n_pts, hand_inds, center, hot_start)
    ...
    def JointVel_Fk(self, f, actions, pos_rot=None):
        ...
        if actions.shape[1] == self.n_actuators_per_hand + 6:
            from mpm.utils import rigid_body_motion_hand
    ...
    def step(self, action, q_state=None):
        pos, rot, q_state = self.compute_forward_kinematics(self.cur, action, pos_rot=q_state)
...
class HandEnv(CudaEnv):
    def __init__(self,
        ...
        self.cfg = cfg
        # SHAPE
        from hand.utils.shape_utils import Shapes
        shape_maker = Shapes(cfg.SHAPES)
        objects, colors, _ = shape_maker.get()
        # expand if necessary
    ...
    def parse_manip_cfgs(self, cfgs):
        ...
        from mpm.hand_utils import DEFAULT_INITIAL_QPOS
    ...
    def parse_sim_cfg(self, cfg):
        ...
        from mpm.hand_utils import prepare
        if mode in ["lh", "rh"]:
            filename = "left_hand" if mode == "lh" else "right_hand"
            n_hands = 1
            root_info, site_computations, joint_info, geometries = prepare(f"shadow/{filename}/shadow_hand.xml",
                                                                           scale=scale)
